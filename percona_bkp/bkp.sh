...
backup_database() {
    ...
    xtrabackup --backup --databases="$db_name" --datadir=/var/lib/mysql/ --target-dir="$backup_dir" --user=root --password="$DB_PASS" --use-memory=5G
...
archive() {
    ...
    tar -cf full_backup_${web_name}_$(date +'%Y%m%d_%H%M').tar --exclude='/srv/web-roots/${web_name}/www/bitrix/backup/' ${db_name}_$(date +'%Y-%m-%d') -C /srv/web-roots ${web_name}
